...
class doawnload_manager:
    ...
    #######################################################################################################
    def ordered_non_loaded_object(self):
        while 1:
            path_choix=random.choice(self.current_data)
            # print(path_choix)
            if (path_choix not in self.loaded)and (path_choix.split("\\")[0]==self.id) :
                self.loaded.append(path_choix)
                return path_choix
    #######################################################################################################
    def load_batch(self,batch_size):
        self.batch=[]
        self.id=self.current_data[0].split("\\")[0]
        print(self.id)
        ...
        for i in range(batch_size):
            ...
            path=self.ordered_non_loaded_object()
            print(path)
            print("*"*50)
            self.download(path)
            print("*"*50)
