...
def _download(url, dst_file, skip_if_exists=True,nbr=0):
    if nbr<3:
        ...
        try:
            ...
            if r.ok:
                ...
                print("Download request failed. trying again ...")
                _download(url, dst_file, skip_if_exists=True,nbr=nbr+1)
                # return False
        except:
            e = sys.exc_info()[0]
...
if __name__ == "__main__":
    # Test cases
    aaaa=random.choice(rgbds)
    #download_all("00001")
    #download_category("atm")
    #print(rgbds,len(rgbds))
    #download_video(aaaa)
    #download_mesh(random.choice(meshes))
    #download_rgbd(aaaa)
    #print(meshes)